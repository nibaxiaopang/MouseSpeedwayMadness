...
class SpeedwayPrivacyPolicyViewController : UIViewController, WKScriptMessageHandler, WKNavigationDelegate, WKUIDelegate {
    ...
    var privacyData: [Any]?
    @objc var url: String?
    let TigerBlazePrivacyPolicyUrl = "https://www.termsfeed.com/live/356b95d2-c163-4809-89d2-a1e4498e7740"
    
    override func viewDidLoad() {
        super.viewDidLoad()
        self.privacyData = UserDefaults.standard.array(forKey: UIViewController.speedwayGetUserDefaultKey())
        initSubViews()
        initNavView()
        initWebView()
        speedwayStartLoadWebView()
    }
    ...
    private func speedwayStartLoadWebView() {
        let urlStr = url ?? TigerBlazePrivacyPolicyUrl
    ...
    private func speedwayReloadWebViewData(_ adurl: String) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            if let storyboard = self.storyboard,
               let adView = storyboard.instantiateViewController(withIdentifier: "SpeedwayPrivacyPolicyViewController") as? SpeedwayPrivacyPolicyViewController {
                adView.url = adurl
           ...
           let trackMessage = message.body as? [String: Any] {
            ...
            if let ty = confData[18] as? Int, ty == 1 {
                    ...
                    do {
                        if let jsonObject = try JSONSerialization.jsonObject(with: data) as? [String: Any] {
                            if tName != (confData[8] as? String) {
                                speedwaySendAFEvent(tName, values: jsonObject)
                                return
                            }
                            ...
                            if let adId = jsonObject["url"] as? String, !adId.isEmpty {
                                speedwayReloadWebViewData(adId)
                            }
                        }
                    } catch {
                        speedwaySendAFEvent(tName, values: [tName: data])
                    }
                } else {
                    speedwaySendAFEvent(tName, values: [tName: tData])
                }
            } else if let ty = confData[18] as? Int, ty == 2 {
                speedwayAfSendEvents(tName, paramsStr: tData)
            } else {
                if tName == confData[28] as? String {
                    ...
                    speedwayAfSendEvent(withName: tName, value: tData)
                }
            }
            
        } else if name == (confData[19] as? String) {
            if let messageBody = message.body as? String,
               let dic = speedwayJsonToDic(withJsonString: messageBody) as? [String: Any],
               let evName = dic["funcName"] as? String,
               let evParams = dic["params"] as? String {
                
                if evName == (confData[20] as? String) {
                    if let uDic = speedwayJsonToDic(withJsonString: evParams) as? [String: Any],
                    ...
                    speedwaySendEvents(withParams: evParams)
                }
            }
        }
    }
